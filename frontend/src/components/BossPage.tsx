 ...
 * FlambeadorBoss, etc.) pasa sus retos, consejos y receta principal.
 *
 * - Cada reto se supera subiendo una foto que `evaluateImage()` puntúa vía el proxy.
 ...
 *
 * El color sale del token `world-N` (prop `world` o deducido de `worldName`).
 * Las props de color antiguas se aceptan pero ya no se usan.
 */
...
export interface BossPageProps {
  ...
  /** Mundo del jefe; define el color. Si falta, se deduce de `worldName`. */
  world?: WorldId;
  ...
  accentTextFaint?: string;
  doneBg?: string;
  doneBorder?: string;
  doneCardBg?: string;
  uploadBorderHover?: string;
...
export const BossPage = ({
  bossName, bossEmoji, bossSubtitle, levelNum, worldName, xpReward, world,
      ...
      if (evalResult.stars && evalResult.stars >= 1) {
        setCompletedChallenges(prev => {
          ...
          if (next.size === challenges.length) {
            setTimeout(() => {
              // Se mira antes de que el efecto guarde las estrellas: al repetir no hay XP.
              setFirstWin(getLevelStars(location.pathname) === 0);
                      ...
                      {!img ? (
                        <label className="block cursor-pointer rounded-card focus-within:ring-2 focus-within:ring-brand-700">
                          ...
                          <input
                            ...
                            accept="image/*"
                            className="sr-only"
                            onChange={e => { const f = e.target.files?.[0]; e.target.value = ''; if (f) handleFile(challenge.id, f); }}
                          />
                        </label>
                      ) : (
